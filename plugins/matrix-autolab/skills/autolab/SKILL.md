...
- Before each phase or meaningful substep, declare the current work with `python plugins/matrix-autolab/scripts/autolab_gate.py start-step --run-id <run_id> ...`
- Paper-derived details must be recorded in `.autolab/runs/<run_id>/requirements.json` with `define-requirement`
- Implementation, test, command, metric, and report evidence must be recorded with `add-evidence`
- A phase or substep is not complete until `check-step` returns `ok: true`
- If `check-step` reports blockers, STOP and fix the blockers instead of continuing to the next phase
...
- [ ] Otherwise:
  ...
  - [ ] Ask user: "Do you need to generate the framework figure? (yes/no)"
    ...
    - If **yes**: Invoke `paperbanana` skill using Skill tool
...
- [ ] Ask user: "AutoLab complete. Do you need to train SOTA baselines for comparison? (yes/no)"
  - If **yes**: Invoke `autobaseline` skill using Skill tool
  - If **no**: End workflow
...
1. Start the gate for the current phase or substep with `autolab_gate.py start-step`
2. Record every paper requirement being handled with `autolab_gate.py define-requirement`
3. Execute only the active gate step
4. Record implementation, test, command, metric, and report evidence with `autolab_gate.py add-evidence`
5. Run `autolab_gate.py check-step --run-id <run_id>` and stop if any blocker remains
6. Present the phase report to the user
7. Explicitly ask: "Phase X complete. Please review `experiment_docs/reports/phase_X_{name}_report.md`. Reply 'confirm' to proceed to Phase X+1, or provide feedback for changes."
...
```bash
python plugins/matrix-autolab/scripts/autolab_gate.py define-requirement --run-id <run_id> --id method.loss.total_loss --phase phase_4.5_loss_check --kind paper --source paper:method-loss --text "Total loss components from the paper must all be implemented and active in backward pass"
python plugins/matrix-autolab/scripts/autolab_gate.py start-step --run-id <run_id> --phase phase_4.5_loss_check --step total_loss_active_check --requirement method.loss.total_loss --required-artifact experiment_docs/reports/phase_4.5_loss_check_report.md --check "all loss components are defined, called, non-zero, and included in backward pass" --user-confirmation-required
